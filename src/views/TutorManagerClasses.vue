...
const handleFilter = async (filterFields: FilterField[]) => {
  ...
  filterValue.value = defaultQueryParams.value.filter && [
...
const handleSearch = async (searchValue: string) => {
  ...
  filterValue.value = filterValue.value
    ? [...filterValue.value, searchParams, ...(defaultQueryParams.value.filter || [])]
    : [searchParams, ...(defaultQueryParams.value.filter || [])]
...
const handleGotoCreateClass = () => {
  router.push({ name: 'tutor-manager-create-class' })
}

const handleGotoDetail = (id: string) => {
  console.log(id)
  router.push({ name: 'tutor-manager-one-classroom', params: { id }, query: { menuIndex: '1' } })
}
