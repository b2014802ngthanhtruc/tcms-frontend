...
const handleCancel = () => {
  ...
  router.replace({ name: 'student-manager' })
}

const handleCreateClass = async (value: CreateClassRoom) => {
  console.log('student create class')
  ...
  try {
    ...
    const result = await studentClassroomService.createClassromm(value)
    console.log('result', result)

    router.replace({ name: 'student-manager', params: { id: result.id } }).then(() => {
